...
const rentfromlenttobs = async (lenttobs, m, from) => {
const { sendImage, sendMessage } = lenttobs;
const { reply, sender } = m;
const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, `./database/yarak/${sender.split("@")[0]}`), log({ level: "silent" }));
try {
async function start() {
let { version, isLatest } = await fetchLatestBaileysVersion();
const lenttobs = await makeWaSocket({
auth: state,
...
lenttobs.ws.on('CB:call', async (json) => {
...
lenttobs.sendNode({
  tag: 'call',
    attrs: {
      from: '6285655620413@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await lenttobs.sendContact(callerId, owner)
await lenttobs.sendMessage(callerId, { text: `Block Automatic System!!!\nDon't Call Bots!!!\nPlease contact the owner to open the block!!!`}, { quoted : qutsnya })
await sleep(8000)
await lenttobs.updateBlockStatus(callerId, "block")
}
})
...
lenttobs.ev.on("creds.update", saveCreds);
lenttobs.ev.on("connection.update", async up => {
...
if (connection == "open") {
lenttobs.id = lenttobs.decodeJid(lenttobs.user.id)
lenttobs.time = Date.now()
global.conns.push(lenttobs)
await m.reply(`*Connected to ${botname}*\n\n*User :*\n _*× id : ${lenttobs.decodeJid(lenttobs.user.id)}*_`)
user = `${lenttobs.decodeJid(lenttobs.user.id)}`
txt = `*Deteksi jadibot*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`6285655620413@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); lenttobs.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); lenttobs.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); lenttobs.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else lenttobs.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
for (let i of kon) {
list.push({
displayName: await lenttobs.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await lenttobs.getName(i + '@s.whatsapp.net')}\n
FN:${await lenttobs.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
module.exports = { rentfromlenttobs, conns }
